...
public class MapMatchingCl {
	...
	public static void main(String[] args) {
		...
		new MatsimNetworkReader(scenario.getNetwork()).readFile("output/network_cl.xml");
		...
		config.setFileName("output/2015-04-ShapeRutas.csv");
		...
		routes.entrySet().stream().limit(100).forEach(entry -> {
			...
			for (ClRoute transitRouteStop : entry.getValue()) {
				if (previous == null || CoordUtils.calcEuclideanDistance(previous.coord, transitRouteStop.coord) > 2*measurementErrorSigma) {
					...
					do {
						myRadius += radius;
						nearestNodes = ((NetworkImpl) scenario.getNetwork()).getNearestNodes(transitRouteStop.coord, myRadius);
		...
		new TransitScheduleWriter(scenario.getTransitSchedule()).writeFile("output/matched-transit-schedule.xml");
	}
