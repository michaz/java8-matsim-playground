...
public class ZoneTrackerTest {
	...
	public void worksWhenOnlyTwoLinksAreFenced() {
		...
		ZoneTracker testee = new ZoneTracker(ScenarioUtils.createScenario(ConfigUtils.createConfig()), new ZoneTracker.LinkToZoneResolver() {

			@Override
			public Id<ZoneTracker.Zone> resolveLinkToZone(Id<Link> linkId) {
				if (linkId.equals(link4) || linkId.equals(link1)) {
					return Id.create(linkId, ZoneTracker.Zone.class);
				} else {
					return null;
				}
			}
