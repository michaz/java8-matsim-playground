...
class MultiRateRunResource {
    ...
    public MultiRateRunResource(String wd) {
        this.WD = wd;
    }
    ...
    BaseRunResource getBaseRun() {
        return new BaseRunResource("/Users/michaelzilske/runs-svn/berlin-bvg09/bvg.run189.10pct", "bvg.run189.10pct");
    }

    public void twoRates(String string) {
        final Scenario baseScenario = getBaseRun().getScenario();
        ...
        for (Person person : baseScenario.getPopulation().getPersons().values()) {
            if (MatsimRandom.getRandom().nextDouble() > 0.1) {
                person.getCustomAttributes().put("phonerate", 0.0);
            }
        }
        ...
        String rateDir = WD + "/rates/" + (int) rate;
        ...
        double originalSampleSize = 0.1;

        new SightingsWriter(sightings).write(rateDir + "/sightings.txt");
        final Counts allCounts = CompareMain.volumesToCounts(baseScenario.getNetwork(), groundTruthVolumes, 1.0 / originalSampleSize);
...
//        new CountsReaderMatsimV1(originalCounts).parse("/Users/michaelzilske/shared-svn/projects/bvg_3_bln_inputdata/rev554B-bvg00-0.1sample/counts/counts.xml.gz");
...
//        }
    ...
    public void errors() {
        ...
        FileIO.writeToFile(filename, new StreamingOutput() {
            ...
            public void write(PrintWriter pw) throws IOException {
                pw.printf("regime\trate\tclonefactor\trms\tmae\tmab\n");
                for (String rate : getRates()) {
                    Collection<String> cloneFactors = getCloneFactors(rate);
                    for (String cloneFactor : cloneFactors) {
                        final playground.mzilske.ant2014.IterationResource lastIteration = getRateRun(rate, cloneFactor).getLastIteration();
                        ...
                        double rootMeanSquare = rootMeanSquare(baseVolumes, volumes);
                        ...
                        pw.printf("%s\t%s\t%s\t%f\t%f\t%f\n",
                                regime, rate, cloneFactor, rootMeanSquare, meanAbsoluteError, meanAbsoluteBias);
                        pw.flush();
                    }
                }
            }
    ...
    private double rootMeanSquare(VolumesAnalyzer baseVolumes, VolumesAnalyzer volumes) {
        double result = 0.0;
        double num = 0.0;
